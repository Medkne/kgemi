...
class TransE(nn.Module):
    ...
    def l2_loss(self):
        return (torch.norm(self.ent_embs.weight, p=2) ** 2)  + (torch.norm(self.rel_embs.weight, p=2) ** 2)

    def forward(self, heads, rels, tails):
        h_embs = self.ent_embs(heads)
        t_embs = self.ent_embs(tails)
        r_embs = self.rel_embs(rels)
        score = torch.norm(h_embs + r_embs - t_embs, p=2, dim=1)  # Ensure this is 1-D
        return score
...
class DistMult(nn.Module):
    ...
    def forward(self, heads, rels, tails):
        ...
        t_embs = self.ent_embs(tails)
        r_embs = self.rel_embs(rels)
        score = torch.sum(t_embs * h_embs * r_embs, dim=1)
        return score
